...
{
	char ReLu[] = "ReLu", Sigmoid[] = "Sigmoid", Softmax[] = "Softmax";
	char *activation_strs[] = {ReLu, Sigmoid, Softmax};
...
double (*NN_activation_funcs[])(double){NN_ReLu, NN_Sigmoid, NN_Softmax};
...
double (*NN_activationDerivative_funcs[])(double){NN_a_ReLuDerivative, NN_a_SigmoidDerivative, NN_a_SoftmaxDerivative};
...
double **NN_process(double ***nn)
{
	for (int layer_no = 2; layer_no < ***nn; layer_no++)
	{
		...
		if (nn[layer_no][0][1] == NN_type_Softmax)
			NN_SoftmaxManager(nn[layer_no]);
	}
	return *(nn + (int)(***nn) - 1);
}
...
= sigma{over all the neurons of next layer}(weight connecting a neuron of next layer*d(Cost)/d(Activation of that neuron of layer no L) * derivative of activation function)*/
	...
	for (int layer_no = 2; layer_no < nn[0][0][0]; layer_no++)
	{
		for (int neuron_no = 1; neuron_no < nn[layer_no][0][0]; neuron_no++)
		{
			...
			for (; weight_no < nn[layer_no][neuron_no][0] - 1; weight_no++)
			{
				gradient[layer_no][neuron_no][weight_no] += nn[layer_no - 1][weight_no - 1][1] * gradient[layer_no][neuron_no][1] * NN_aActivationDerivativef(nn[layer_no][neuron_no][1], nn[layer_no]);
			}
			gradient[layer_no][neuron_no][weight_no] += gradient[layer_no][neuron_no][1] * NN_aActivationDerivativef(nn[layer_no][neuron_no][1], nn[layer_no]);
		}
	}
}
class NN_thread;
...
{
	for (int layer_no = 2; layer_no < ***thread_ptr->nn; layer_no++)
	{
		...
		thread_ptr->joinAll();
		if (thread_ptr->nn[layer_no][0][1] == NN_type_Softmax)
			NN_SoftmaxManager(thread_ptr->nn[layer_no]);
	}
