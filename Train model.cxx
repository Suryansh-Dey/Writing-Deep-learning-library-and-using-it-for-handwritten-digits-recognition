...
void randNoice(double gray_vals[784], float probability)
{
	for (int i = 0; i < 784; i++)
	{
		if (float(rand()) / INT_MAX < probability)
			gray_vals[i] = float(rand()) / INT_MAX;
	}
}
constexpr int BATCH_SIZE = 200, TOTAL_BATCHES = 30, MAX_MOVE = 4;
constexpr float MAX_SCALE = 0.5;
constexpr int EPOCH = 400;
...
int main(int argc, char *argv[])
{
	...
	int neuron_counts[] = {5, 784, 30 ,16 , 10};
	NN model = NN_create(neuron_counts);
	NN_set_activationFunction(model, 4, NN_type_Softmax);
	NN_mutate(model, 1, 0.1); // Initialising with random weights
//	double ***model = NN_load("error.txt");
	NN_set_learningRate(0.01);
	...
	for (int epoch_no = 0; epoch_no < EPOCH; epoch_no++)
	{
		...
		for (int batch_no = 1; batch_no < TOTAL_BATCHES + 1; batch_no++)
		{
			...
			for (int image_no = 0; image_no < BATCH_SIZE; image_no++)
			{
				...
				randNoice(gray_vals[image_no]+1,0.01);
			}
